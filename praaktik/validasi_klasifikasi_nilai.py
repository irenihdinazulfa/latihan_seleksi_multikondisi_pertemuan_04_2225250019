...
try:
    ...
    if not (0 <= ujian <= 100):
        ...
        # Periksa syarat kehadiran minimal 80 persen
        if hadir < 80:
            print("Tidak memenuhi syarat kehadiran")
        else:
            # Tentukan predikat dengan rantai elif menurun
            if akhir >= 85:
                predikat = "A"
            elif akhir >= 70:
                predikat = "B"
                ...
                predikat = "E"
            # Tentukan status lulus atau belum lulus dari predikat
            if predikat in ["A", "B", "C"]:
                status = "Lulus"
            else:
                status = "Belum lulus"
            # Tampilkan predikat dan status
